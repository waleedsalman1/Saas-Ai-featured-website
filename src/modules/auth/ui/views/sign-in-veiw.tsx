  ...
  useEffect(() => {
    ...
    const handleMouseOver = (e: MouseEvent) => {
      ...
      if (['INPUT', 'TEXTAREA'].includes(target.tagName) && cursorRef.current) {
        cursorRef.current.style.opacity = '0'; // Hide dot on text inputs
      }
    };
    ...
    const handleMouseOut = (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      if (['INPUT', 'TEXTAREA'].includes(target.tagName) && cursorRef.current) {
        cursorRef.current.style.opacity = '1'; // Show dot again
      }
    };
      ...
      <div className="relative min-h-screen w-full bg-[#030014] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(80,0,200,0.15),rgba(255,255,255,0))] overflow-hidden flex items-center justify-center">
        ...
        >
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-purple-200/50 mt-2 text-sm">Sign in to continue your journey.</p>
          </div>
          ...
          {(Object.keys(errors).length > 0 || authError) && (
            <div className="mb-6 flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm shadow-[0_0_15px_rgba(239,68,68,0.1)]">
              ...
              <div>
                ...
                <span className="text-red-400/90">{authError || "Please check your credentials."}</span>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3 relative z-20">
            ...
            <button onClick={() => handleSocialLogin('linkedin')} type="button" className="flex items-center justify-center gap-2 py-3.5 rounded-xl bg-white/[0.02] border border-white/5 text-gray-300 hover:bg-white/[0.05] hover:text-white transition-all duration-300 hover:border-purple-500/40 hover:shadow-[0_0_15px_rgba(168,85,247,0.15)] cursor-none text-sm font-medium">
              <svg className="w-5 h-5 shrink-0 text-[#0A66C2]" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div className="mt-8 text-center relative z-20">
            <p className="text-gray-400 text-sm">
              ...
              <Link href="/sign-up" className="text-purple-400 font-medium hover:text-white transition-colors duration-300 hover:drop-shadow-[0_0_10px_rgba(192,132,252,0.9)] cursor-none underline underline-offset-4">
                Sign Up
              </Link>
            </p>
          </div>
